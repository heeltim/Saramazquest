...
test("toda classe possui configuração magic_points", () => {
  const data = loadSrdData();
  for (const cls of data.classes) {
    assert.ok(cls.magic_points && typeof cls.magic_points === "object", `Classe ${cls.id} sem magic_points.`);
    assert.equal(typeof cls.magic_points.enabled, "boolean", `Classe ${cls.id} magic_points.enabled inválido.`);
  }
});
