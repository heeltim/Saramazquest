...
let room = "arena";
let currentUser = prompt("Digite seu nome:") || ("Jogador"+Math.floor(Math.random()*1000));
...
const RACES = {
  Humano: {
    ...
    abilities: [
      { icon:"🌍", name:"Adaptável", desc:"+1 em todos os atributos.", manaCost: 0 },
      { icon:"🔥", name:"Determinação", desc:"1 vez por turno, pode repetir uma ação/rolagem (futuro).", manaCost: 0 }
    ]
  },
  Elfo: {
    bonus: { forca: 0, destreza: 2, espirito: 1 },
    abilities: [
      { icon:"🌙", name:"Visão Noturna", desc:"Enxerga no escuro (futuro).", manaCost: 0 },
      { icon:"✨", name:"Precisão Élfica", desc:"+1 em ações de precisão (futuro).", manaCost: 0 }
    ]
  },
  Anao: {
    bonus: { forca: 2, destreza: 0, espirito: 1 },
    abilities: [
      { icon:"🛡️", name:"Resistência Anã", desc:"Redução de dano físico (futuro).", manaCost: 0 },
      { icon:"⛰️", name:"Vigor de Pedra", desc:"+2 HP por nível (já entra no HP Máx).", manaCost: 0 }
    ]
  }
};
...
const CLASSES = {
  Guerreiro: {
    ...
    manaMod: -2,
    abilities: [
      { icon:"⚔️", name:"Golpe Poderoso", desc:"+3 dano no próximo ataque (futuro).", manaCost: 0 },
      { icon:"🛡️", name:"Postura Defensiva", desc:"+2 defesa por 1 turno (futuro).", manaCost: 0 }
    ]
  },
  Mago: {
    ...
    manaMod: 10,
    abilities: [
      { icon:"🔥", name:"Bola de Fogo", desc:"Ataque mágico (futuro).", manaCost: 3 },
      { icon:"🔮", name:"Escudo Arcano", desc:"Absorve dano (futuro).", manaCost: 2 }
    ]
  },
  Arqueiro: {
    ...
    manaMod: 2,
    abilities: [
      { icon:"🏹", name:"Tiro Preciso", desc:"+4 precisão (futuro).", manaCost: 1 },
      { icon:"👣", name:"Passo Sombrio", desc:"Movimento tático (futuro).", manaCost: 1 }
    ]
  }
};
...
/* ================= BANCO: ITENS (resumo) ================= */
const ITEM_DB = {
  dagger: { name:"Adaga", icon:"🗡️", type:"weapon", equipSlot:"weapon",
    desc:"+1 FOR (arma leve).", mods:{ str:+1 } },
  short_sword: { name:"Espada Curta", icon:"⚔️", type:"weapon", equipSlot:"weapon",
    desc:"+2 FOR (padrão).", mods:{ str:+2 } },
  arcane_staff: { name:"Cajado Arcano", icon:"🪄", type:"weapon", equipSlot:"weapon",
    desc:"+2 ESP (arma de mago).", mods:{ spr:+2 } },

  icebolt: { name:"Raio de Gelo", icon:"❄️", type:"weapon", equipSlot:"weapon",
    desc:"+2 ESP. Magia básica (sem custo).", mods:{ spr:+2 } },
  firebolt: { name:"Faísca de Fogo", icon:"🔥", type:"weapon", equipSlot:"weapon",
    desc:"+2 ESP. Magia básica (sem custo).", mods:{ spr:+2 } },

  leather_armor: { name:"Armadura de Couro", icon:"🥋", type:"armor", equipSlot:"armor",
    desc:"+1 Defesa.", mods:{ defense:+1 } },
  chainmail: { name:"Cota de Malha", icon:"⛓️", type:"armor", equipSlot:"armor",
    desc:"+2 Defesa.", mods:{ defense:+2 } },
  shield: { name:"Escudo", icon:"🛡️", type:"shield", equipSlot:"shield",
    desc:"+1 Defesa.", mods:{ defense:+1 } },
  heavy_shield: { name:"Escudo Pesado", icon:"🛡️", type:"shield", equipSlot:"shield",
    desc:"+2 Defesa.", mods:{ defense:+2 } },

  ring_protection: { name:"Anel de Proteção", icon:"💍", type:"ring", equipSlot:"ring",
    desc:"+1 Defesa.", mods:{ defense:+1 } },
  elven_cloak: { name:"Capa Élfica", icon:"🧥", type:"cloak", equipSlot:"cloak",
    desc:"+1 DES.", mods:{ dex:+1 } },
  backpack: { name:"Mochila", icon:"🎒", type:"misc", equipSlot:"misc",
    desc:"+6 slots inventário.", mods:{ invExtra:+6 } },

  potion_healing: { name:"Poção de Cura", icon:"🧪", type:"potion",
    desc:"+10 HP (consome).", consume:{ hp:+10 } },
  potion_mana: { name:"Poção de Mana", icon:"🔵", type:"potion",
    desc:"+8 MP (consome).", consume:{ mana:+8 } }
};
const SHOP = [
  { id:"dagger", price: 8 },
...
/* ================= STORAGE ================= */
function load(){
  return JSON.parse(localStorage.getItem(STORAGE_KEY)) || { rooms:{}, chat:{}, scenes:{} };
}
function save(data){
...
function ensurePlayerSchema(p){
  ...
  if(p.equipped === undefined) p.equipped = { weapon:null, armor:null, shield:null, ring:null, cloak:null, misc:null };
...
function pushAction(user, text){ pushChat(user, "* " + text); }
...
function computeItemMods(p){
  ...
  const mods = { str:0, dex:0, spr:0, defense:0, hpMax:0, manaMax:0, invExtra:0 };
  ids.forEach(id=>{
    const it = ITEM_DB[id];
...
function invCount(p){ return (p.inventory || []).length; }
function invMax(p){ return p.invMax || 12; }
...
function ensureMageFreeWeapon(p){
  ...
  if(!p.equipped) p.equipped = { weapon:null, armor:null, shield:null, ring:null, cloak:null, misc:null };
...
function recalcFromSheet(p){
  ...
  let destreza = base.destreza + (bonusR.destreza||0) + (bonusC.destreza||0);
  let espirito = base.espirito + (bonusR.espirito||0) + (bonusC.espirito||0);
...
if(!data.rooms[room][currentUser]){
  ...
  data.rooms[room][currentUser]={
    ...
    equipped:{ weapon:null, armor:null, shield:null, ring:null, cloak:null, misc:null },
    color:randomColor(),
    onTable:true
  };
...
function updateArena(){
  ...
  Object.keys(players).forEach(name=>{
    ...
    if(getTile(p.x,p.y) !== "floor"){
      ...
      if(found){ p.x = found.x; p.y = found.y; }
    }
  ...
  if(sheetTargetName){
    ...
    if(p) { ensurePlayerSchema(p); recalcFromSheet(p); renderSheetComputed(p); renderEquip(p); renderAbilities(p); }
  }
  if(invTargetName){
    const p = load().rooms[room][invTargetName];
    if(p) { ensurePlayerSchema(p); recalcFromSheet(p); renderInventoryModal(p); }
  }
}
...
function addBackToTable(name){
  ...
  const s = data.scenes[room];
  const found = findNearestFloor(Math.floor(s.cols/2), Math.floor(s.rows/2));
  if(found){ p.x = found.x; p.y = found.y; }
  p.onTable=true;
...
function editStat(name, stat){
  ...
  let v = prompt(`Digite um valor para ${stat.toUpperCase()} (ex: -20 ou +10):`, "-10");
  if(v === null) return;
...
function openSheet(name){
  ...
  raceSel.innerHTML = Object.keys(RACES).map(r=>`<option value="${r}">${r}</option>`).join("");
  classSel.innerHTML = Object.keys(CLASSES).map(c=>`<option value="${c}">${c}</option>`).join("");

  document.getElementById("sheetTitle").textContent = `Ficha — ${name}`;
  document.getElementById("sheetSub").textContent = `Equipados ficam aqui. Inventário/Loja ficam no 🎒.`;
...
function renderSheetComputed(p){
  ...
  document.getElementById("statDEF").textContent = p.defense ?? (10 + p.attributes.destreza);
...
function renderAbilities(p){
  ...
  list.innerHTML = skills.map((a, idx)=>`
    <div class="ability">
...
function useAbility(skillIndex){
  ...
  pushAction(currentUser, `${sheetTargetName} usou ${a.icon} ${a.name}${cost>0 ? ` (-${cost} MP)` : ""}.`);
  updateArena();
...
function renderEquip(p){
  const eq = p.equipped || { weapon:null, armor:null, shield:null, ring:null, cloak:null, misc:null };
...
function openInventory(name){
  ...
  document.getElementById("invSub").textContent = `Ações via menu ⋯. Loja com ouro. Itens equipados refletem na ficha.`;

  const filter = document.getElementById("shopFilter");
  filter.onchange = ()=> {
    const p2 = load().rooms[room][invTargetName];
    if(p2){ ensurePlayerSchema(p2); recalcFromSheet(p2); renderInventoryModal(p2); }
  };
...
function renderInventoryModal(p){
  ...
  document.getElementById("invSlotsValue").textContent = `${invCount(p)}/${invMax(p)}`;
  renderInvList(p);
  renderShop(p);
}
...
function menuSend(itemId){
  ...
  const names = Object.keys(data.rooms[room]).filter(n=>n !== invTargetName);
  if(names.length === 0){
  ...
  const it = ITEM_DB[itemId];
  if(it && it.equipSlot && from.equipped && from.equipped[it.equipSlot] === itemId){
  ...
  pushAction(currentUser, `${invTargetName} entregou ${it.icon} ${it.name} para ${targetName}.`);
...
function setTool(tool){
  paintTool = tool;
  document.getElementById("toolFloor").classList.toggle("active", tool==="floor");
  document.getElementById("toolWall").classList.toggle("active", tool==="wall");
  document.getElementById("toolVoid").classList.toggle("active", tool==="void");
}
