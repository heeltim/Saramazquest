...
function sendMessage() {
  ...
  const handledRoll = handleRollCommand(text);
  if (!handledRoll) {
...
const DICE_TYPES = DICE_META.map((die) => die.sides);
let diceSelection = Object.fromEntries(DICE_TYPES.map((dice) => [dice, 0]));

function renderDiceTray() {
  const diceTray = document.getElementById("diceTray");
  if (!diceTray) return;

  diceTray.innerHTML = DICE_META.map((die) => {
    const { sides, icon } = die;
    const qty = diceSelection[sides] || 0;
    return `
      <div class="diceChip">
        <div class="diceFace">
          <span class="diceIcon">${icon}</span>
          <span class="diceLabel">D${sides}</span>
        </div>
        <div class="diceControls">
          <button class="diceControlBtn" onclick="changeDiceQty(${sides}, -1)">−</button>
          <span class="diceQty">${qty}</span>
          <button class="diceControlBtn" onclick="changeDiceQty(${sides}, 1)">+</button>
        </div>
      </div>
    `;
...
function changeDiceQty(dice, delta) {
  const current = diceSelection[dice] || 0;
  const next = Math.max(0, Math.min(20, current + delta));
  diceSelection[dice] = next;
  renderDiceTray();
}

function clearDiceSelection() {
  diceSelection = Object.fromEntries(DICE_TYPES.map((dice) => [dice, 0]));
  renderDiceTray();
}
...
function rollSelectedDice() {
  const picked = DICE_TYPES.filter((dice) => (diceSelection[dice] || 0) > 0);
  if (!picked.length) {
    alert("Escolha pelo menos 1 dado para rolar.");
    return;
  }
  ...
  picked.forEach((dice) => {
    const qty = diceSelection[dice];
    const results = Array.from({ length: qty }, () => rollDie(dice));
    const sum = results.reduce((acc, value) => acc + value, 0);
    total += sum;
    groups.push(`${qty}d${dice} [${results.join(", ")}]`);
  });

  pushAction(
    currentUser,
    `rolou 🎲 ${groups.join(" + ")} = <strong>${total}</strong>.`,
  );
}

function handleRollCommand(text) {
  const normalized = text.trim();
  const match = normalized.match(/^\/(r|roll)\s+(.+)$/i);
  if (!match) return false;

  const expression = match[2].replace(/\s+/g, "");
  const groups = expression.split("+").filter(Boolean);
  if (!groups.length) {
    alert("Use /roll no formato: /roll 2d6+1d20");
    return true;
  }
  ...
  for (const group of groups) {
    const g = group.match(/^(\d*)d(4|6|8|10|12|20)$/i);
    if (!g) {
      alert("Formato inválido. Exemplo: /roll 2d6+1d20");
      return true;
    }

    const qty = Math.max(1, Math.min(20, parseInt(g[1] || "1", 10)));
    const sides = parseInt(g[2], 10);
    const results = Array.from({ length: qty }, () => rollDie(sides));
    const sum = results.reduce((acc, value) => acc + value, 0);
    total += sum;
    outputGroups.push(`${qty}d${sides} [${results.join(", ")}]`);
  }

  pushAction(
    currentUser,
    `rolou 🎲 ${outputGroups.join(" + ")} = <strong>${total}</strong>.`,
  );
  return true;
}
